...
function nonNullable(condition, debugPath = "variable") {
  if (!condition) throw new Error(`${debugPath} cannot be nil`);
}

function validateConfig(config) {
  if (typeof config.SCRIPTS !== "object" || config.SCRIPTS == null) throw new Error();

  for (const scriptName of Object.keys(config.SCRIPTS)) {
    const scriptConfig = config.SCRIPTS[scriptName];
    nonNullable(scriptConfig === null || scriptConfig === void 0 ? void 0 : scriptConfig.CODE_HASH);
    ...
    if (depType !== "dep_group" && depType !== "code") {
      throw new Error(`SCRIPTS.${scriptName}.DEP_TYPE must either be dep_group or code!`);
    }
