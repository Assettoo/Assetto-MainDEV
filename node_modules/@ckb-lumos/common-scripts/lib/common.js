...
async function transfer(txSkeleton, fromInfos, toAddress, amount, changeAddress, tipHeader, {
  ...
  const targetLockScriptInfo = lockScriptInfos.infos.find(lockScriptInfo => {
    return lockScriptInfo.code_hash === toScript.code_hash && lockScriptInfo.hash_type === toScript.hash_type;
  });
...
async function _commonTransferCompatible(txSkeleton, fromInfos, amount, minimalChangeCapacity, {
  ...
  const getInputKey = input => {
    var _input$out_point, _input$out_point2;

    return `${(_input$out_point = input.out_point) === null || _input$out_point === void 0 ? void 0 : _input$out_point.tx_hash}_${(_input$out_point2 = input.out_point) === null || _input$out_point2 === void 0 ? void 0 : _input$out_point2.index}`;
  };
...
function _deductCapacityCompatible(txSkeleton, fromScript, capacity) {
  ...
  for (; i < txSkeleton.get("outputs").size && _capacity.gt(0); i++) {
    ...
    if (output && new ScriptValue(output.cell_output.lock, {
      validate: false
  ...
  txSkeleton = txSkeleton.update("outputs", outputs => {
    return outputs.filter(output => _bi.BI.from(output.cell_output.capacity).toString() !== _bi.BI.from(0).toString());
  });
...
async function collectInputCompatible(txSkeleton, inputCell, fromInfo, {
  ...
  const lastOutputCapacity = _bi.BI.from(lastOutput.cell_output.capacity);
  ...
  if (config.SCRIPTS.ANYONE_CAN_PAY && (0, _helper.isAcpScript)(fromScript, config)) {
    ...
    if (destroyable) {
      ...
      clonedLastOutput.cell_output.capacity = "0x" + outputCapacity.toString(16);
      txSkeleton = txSkeleton.update("outputs", outputs => {
    ...
    if (lastOutputFixedEntryIndex < 0) {
      ...
      // Remove last output
      availableCapacity = _bi.BI.from((_txSkeleton$get$get = txSkeleton.get("outputs").get(lastOutputIndex)) === null || _txSkeleton$get$get === void 0 ? void 0 : _txSkeleton$get$get.cell_output.capacity);
      txSkeleton = txSkeleton.update("outputs", outputs => {
        return outputs.remove(lastOutputIndex);
      });
    }
...
async function setupInputCell(txSkeleton, inputCell, fromInfo, {
  ...
  const targetLockScriptInfo = lockScriptInfos.infos.find(lockScriptInfo => {
    return lockScriptInfo.code_hash === inputLock.code_hash && lockScriptInfo.hash_type === inputLock.hash_type;
  });
