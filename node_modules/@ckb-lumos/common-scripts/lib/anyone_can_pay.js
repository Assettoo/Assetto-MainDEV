...
async function setupInputCell(txSkeleton, inputCell, _fromInfo, {
  ...
  const firstIndex = txSkeleton.get("inputs").findIndex(input => {
    return new ScriptValue(input.cell_output.lock, {
      validate: false
...
async function setupOutputCell(txSkeleton, outputCell, {
  ...
  const capacity = _bi.BI.from(outputCell.cell_output.capacity);
...
async function injectCapacity(cellCollector, txSkeleton, outputIndex, capacity, {
  ...
  for (; i < txSkeleton.get("outputs").size && _capacity.gt(0); i++) {
    ...
    if (new ScriptValue(output.cell_output.lock, {
      ...
      output.cell_output.capacity = "0x" + cellCapacity.sub(deductCapacity).toString(16);
    }
...
function prepareSigningEntries(txSkeleton, {
  ...
  for (let i = 0; i < inputs.size; i++) {
    ...
    if (template.CODE_HASH === input.cell_output.lock.code_hash && template.HASH_TYPE === input.cell_output.lock.hash_type && !processedArgs.has(input.cell_output.lock.args)) {
      processedArgs = processedArgs.add(input.cell_output.lock.args); // skip if input sum capcity <= output sum capacity
      ...
      const outputs = txSkeleton.get("outputs").filter(output => {
        return new ScriptValue(output.cell_output.lock, {
          validate: false
        }).equals(new ScriptValue(input.cell_output.lock, {
          validate: false
        }));
      });
      const sumOfOutputCapacity = outputs.map(output => _bi.BI.from(output.cell_output.capacity)).reduce((result, c) => result.add(c), _bi.BI.from(0));
      const sumOfOutputAmount = outputs.filter(output => output.data !== "0x").map(output => _bi.BI.from(readBigUInt128LECompatible(output.data))).reduce((result, c) => result.add(c), _bi.BI.from(0));
      const fInputs = inputs.filter(i => {
        return new ScriptValue(i.cell_output.lock, {
          validate: false
        }).equals(new ScriptValue(input.cell_output.lock, {
          validate: false
        }));
      });
      const sumOfInputCapacity = fInputs.map(i => _bi.BI.from(i.cell_output.capacity)).reduce((result, c) => result.add(c), _bi.BI.from(0));
      const sumOfInputAmount = fInputs.filter(i => i.data !== "0x").map(i => _bi.BI.from(readBigUInt128LECompatible(i.data))).reduce((result, c) => result.add(c), _bi.BI.from(0));
...
async function withdraw(txSkeleton, fromInput, toAddress, capacity, {
  ...
  if (!(0, _helper.isAcpScript)(fromInput.cell_output.lock, config)) {
  ...
  const fromInputCapacity = _bi.BI.from(fromInput.cell_output.capacity);
  ...
  if ((0, _helper.isAcpScript)(toScript, config)) {
    ...
    const outputCapacity = _capacity.add(toAddressInput.cell_output.capacity);
  ...
  txSkeleton = await setupInputCell(txSkeleton, fromInput, (0, _helpers.generateAddress)(fromInput.cell_output.lock, {
    config
  }), {
    config
