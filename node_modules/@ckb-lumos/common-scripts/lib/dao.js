...
async function withdraw(txSkeleton, fromInput, fromInfo, {
  ...
  if (!typeScript || typeScript.code_hash !== DAO_SCRIPT.CODE_HASH || typeScript.hash_type !== DAO_SCRIPT.HASH_TYPE || fromInput.data !== DEPOSIT_DAO_DATA) {
  ...
  clonedTargetOutput.data = toBigUInt64LE(_bi.BI.from(fromInput.block_number));
  txSkeleton = txSkeleton.update("outputs", outputs => {
...
async function unlock(txSkeleton, depositInput, withdrawInput, toAddress, fromInfo, {
  ...
  if (!typeScript || typeScript.code_hash !== DAO_SCRIPT.CODE_HASH || typeScript.hash_type !== DAO_SCRIPT.HASH_TYPE || depositInput.data !== DEPOSIT_DAO_DATA) {
    throw new Error("depositInput is not a DAO deposit cell.");
  }

  const withdrawTypeScript = withdrawInput.cell_output.type;

  if (!withdrawTypeScript || withdrawTypeScript.code_hash !== DAO_SCRIPT.CODE_HASH || withdrawTypeScript.hash_type !== DAO_SCRIPT.HASH_TYPE || withdrawInput.data === DEPOSIT_DAO_DATA) {
  ...
  const depositEpoch = parseEpochCompatible(depositBlockHeader.epoch); // const depositCapacity = BigInt(depositInput.cell_output.capacity)
  ...
  const defaultWitness = new _toolkit.Reader(_base.core.SerializeWitnessArgs(_toolkit.normalizers.NormalizeWitnessArgs(defaultWitnessArgs))).serializeJson();
...
function extractDaoDataCompatible(dao) {
  ...
  return ["c", "ar", "s", "u"].map((key, i) => {
    return {
      [key]: _bi.BI.from((0, _utils.readBigUInt64LECompatible)("0x" + hex.slice(len * i, len * (i + 1))))
    };
...
function calculateMaximumWithdrawCompatible(withdrawCell, depositDao, withdrawDao) {
  ...
  const outputCapacity = _bi.BI.from(withdrawCell.cell_output.capacity);
