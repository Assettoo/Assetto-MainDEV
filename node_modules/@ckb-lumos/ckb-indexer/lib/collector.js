...
class CKBCellCollector {
  constructor(indexer, queries, otherQueryOptions) {
  ...
  convertQueryOptionToSearchKey() {
    ...
    if (this.queries.lock) {
      if ((0, _services.instanceOfScriptWrapper)(this.queries.lock)) {
        _toolkit.validators.ValidateScript(this.queries.lock.script);
  ...
  async getLiveCell(lastCursor) {
    ...
    const result = await this.indexer.getCells((0, _services.generateSearchKey)(this.queries), undefined, searchKeyFilter);
    return result;
  }
  ...
  shouldSkipped(cell, skippedCount = 0) {
    ...
    if (this.queries.argsLen !== -1 && this.queries.argsLen !== "any" && (0, _services.getHexStringBytes)(cell.cell_output.lock.args) !== this.queries.argsLen) {
      return true;
    }
  }
